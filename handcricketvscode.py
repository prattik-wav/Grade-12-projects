    ...
    def save_profile(self, profile: dict, player_name: str = "", password: str = None):
        ...
        sql = """
              ...
              (name, password, lifetime_runs, lifetime_wickets, total_matches,
               total_wins, total_losses, total_draws, avg_runs, total_balls_faced, total_balls_bowled, centuries, half_centuries)
              ...
              ON DUPLICATE KEY UPDATE
                ...
                total_balls_faced = VALUES(total_balls_faced),
                total_balls_bowled = VALUES(total_balls_bowled),
        ...
        try:
            ...
            cursor.execute("""
                SELECT lifetime_runs, lifetime_wickets, total_matches, 
                    ...
                    total_balls_faced, total_balls_bowled, centuries, half_centuries
...
class GameDataManager:
    ...
    def get_current_match_data(self, game_var):
        ...
        player_match_wickets_taken = game_var.get('lifetime_wickets', 0) 
...
class GameManager:
    ...
    def handle_profile_display_and_save(self, game_var):
        ...
        lifetime_strike_rate = self.gameplay.calculate_strike_rate(game_var['lifetime_runs'], game_var['lifetime_balls_faced'])

        lifetime_economy_rate = self.gameplay.calculate_economy_rate(game_var['lifetime_runs_conceded'], game_var['lifetime_balls_bowled'])
        ...
        self.db_handler.save_profile(game_var['player_profile'], self.name, password)
...
class LeaderboardManager:
    ...
    def show_menu(self):
        while True:
            ...
            print("6. Lifetime Run Rate Leaderboard")
            print("7. Win Rate Leaderboard")
            print("8. All-Rounders Leaderboard")
            print("9. Lifetime Strike Rate Leaderboard") 
            print("10. Lifetime Economy Rate Leaderboard") 
            print("11. Centuries Leaderboard") 
            print("12. Half Centuries Leaderboard")
            ...
            options = {
                ...
                "6": self.show_lifetime_run_rate,
                "7": self.show_win_rate,
                "8": self.show_all_rounders,
                "9": self.show_lifetime_strike_rate, 
                "10": self.show_lifetime_economy_rate, 
                "11": self.show_centuries, 
                "12": self.show_half_centuries 
            }
    ...
    def show_lifetime_run_rate(self):
        ...
        cursor.execute("""
            SELECT player_name,
                   ...
                   CASE 
                       ...
                       THEN ROUND((SUM(runs) * 6.0) / SUM(balls_faced), 2)
                       ELSE 0.0
                   END as lifetime_run_rate
            FROM match_data
            GROUP BY player_name
            HAVING total_balls > 0
        ...
        for i, row in enumerate(results, 1):
            player_name, total_runs, total_balls, run_rate = row
            print(f"{i:<6}{player_name.upper():<15}{run_rate:<12}{total_runs}/{total_balls}")
    ...
    def show_lifetime_strike_rate(self):
        ...
        cursor.execute("""
            SELECT name,
                   ...
                   CASE
                       ...
                       THEN ROUND((lifetime_runs * 100.0) / total_balls_faced, 2)
                       ELSE 0.0
        ...
        for i, row in enumerate(results, 1):
            player_name, total_runs, total_balls_faced, strike_rate = row
            print(f"{i:<6}{player_name.upper():<15}{strike_rate:<10}{total_runs}/{total_balls_faced}")
    ...
    def show_lifetime_economy_rate(self):
        ...
        cursor.execute("""
            SELECT name,
                   lifetime_runs_conceded, -- Assuming you add this to player_profile
                   total_balls_bowled,
                   CASE
                       WHEN total_balls_bowled > 0
                       THEN ROUND((lifetime_runs_conceded * 6.0) / total_balls_bowled, 2)
                       ELSE 0.0
                   END as lifetime_economy_rate
            FROM player_profile
            WHERE total_balls_bowled > 0
            ORDER BY lifetime_economy_rate ASC -- Lower economy rate is better
            LIMIT 10
        """)
        results = cursor.fetchall()
        ...
        for i, row in enumerate(results, 1):
            player_name, runs_conceded, balls_bowled, economy_rate = row
            print(f"{i:<6}{player_name.upper():<15}{economy_rate:<10}{runs_conceded}/{balls_bowled}")

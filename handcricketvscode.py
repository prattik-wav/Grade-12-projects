...
class GamePlay:
    ...
    def check_milestones(self, current_runs, prev_runs, game_state):
        ...
        old = game_state.current_match_highscore

        if current_runs > old:
            previous_temp_flag = game_state.temp_high_score_achievement_unlocked
            game_state.current_match_highscore = current_runs

            if current_runs > game_state.current_match_highscore and not previous_temp_flag:
                
                game_state.temp_high_score_achievement_unlocked = True
                
                temp_ach = Panel.fit(f"[bold green]🏆 Temporary Achievement Unlocked:[/bold green]\n"
                                    f"[bold red]New High Score in match![/bold red]\n"
                                    f"You've just set a new personal high score of {current_runs} runs in this match!", 
                                    border_style="bright_blue")
                console.print(temp_ach)
                sounds.play_achievement()

            elif current_runs > game_state.current_match_highscore and previous_temp_flag:
                game_state.temp_high_score_achievement_unlocked = False
...
class GameManager:
    ...
    def result(self, game_state):
        ...
        achievement_manager.check_achievements(game_state)
    ...
    def handle_profile_display_and_save(self, game_state, current_match_data=None):
        ...
        console.print("\n[bold magenta]Achievements Unlocked[/bold magenta]")
...
class LeaderboardManager:
    ...
    def show_menu(self):
        while True:
            ...
            sounds.play_click()
            options = {
                "00": self.leaderboard_info,
                "01": self.show_runs,
                "02": self.show_wickets,
                "03": self.show_balls_played,
                "04": self.show_most_active,
                "05": self.show_high_scores,
                "06": self.show_win_rate,
                "07": self.show_all_rounders,
                "08": self.show_lifetime_run_rate,
                "09": self.show_lifetime_strike_rate, 
...
class SoundManager:
    def __init__(self):
        ...
        try:
            pygame.mixer.init()
            pygame.mixer.music.load("assets/undertalebg.wav")
        ...
        if self.enabled and os.path.exists("assets/undertalebg.wav"):
            try:
                pygame.mixer.music.load("assets/undertalebg.wav")
            except Exception:
                self.enabled = False  

        self.milestone = _safe("assets/cheer.wav")
        self.wicket   = _safe("assets/out.wav")
        self.score    = _safe("assets/score.wav")
        self.click    = _safe("assets/click.wav")
        self.run      = _safe("assets/run.wav")
        self.achievement = _safe("assets/achievement.wav")
...
def main(game_manager, game_state):
    ...
    while True:
        ...
        if choice == "tutorial":
            ...
            leaderboard_manager.show_menu(achievement_manager)

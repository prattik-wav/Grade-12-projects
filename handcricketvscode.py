...
class GamePlay:
    ...
    def check_milestones(self, current_runs, prev_runs, game_state):
        ...
        if current_runs > old:
            previous_temp_flag = bool(game_state.temp_high_score_achievement_unlocked)
            game_state.current_match_highscore = current_runs

            if not previous_temp_flag:
                game_state.temp_high_score_achievement_unlocked = True
                temp_ach = Panel.fit(
                    f"[bold green]🏆 Temporary Achievement Unlocked:[/bold green]\n"

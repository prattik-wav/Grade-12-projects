class HandCricketDB:
    ...
    def save_profile(self, profile: dict, player_name: str = ""):
    ...
    def show_leaderboard(self):
        try:
            ...
            cursor.execute("""
                SELECT player_name, SUM(runs) as total_runs, MIN(timestamp)
                FROM match_data
                GROUP BY player_name
                ORDER BY total_runs DESC, MIN(timestamp)
                LIMIT 5
            """)
            rows = cursor.fetchall()
            print("TOTAL RUNS SCORED".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'RUNS':<10}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[1]:<10}")

            cursor.execute("""
                SELECT player_name, SUM(wickets) as total_wickets, MIN(timestamp)
                FROM match_data
                GROUP BY player_name
                ORDER BY total_wickets DESC, MIN(timestamp)
                LIMIT 5
            """)
            rows = cursor.fetchall()
            print("\nTOTAL WICKETS TAKEN".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'WICKETS':<10}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[1]:<10}")

            cursor.execute("""
                SELECT player_name, SUM(balls_faced) as total_balls, MIN(timestamp)
                FROM match_data
                GROUP BY player_name
                ORDER BY total_balls DESC, MIN(timestamp)
                LIMIT 5
            """)
            rows = cursor.fetchall()
            print("\nBALLS PLAYED".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'BALLS':<10}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[1]:<10}")

            cursor.execute("""
                SELECT player_name, COUNT(*) as matches, MIN(timestamp)
                FROM match_data
                GROUP BY player_name
                ORDER BY matches DESC, MIN(timestamp)
                LIMIT 5
            """)
            rows = cursor.fetchall()
            print("\nMOST ACTIVE PLAYERS".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'MATCHES':<10}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[1]:<10}")

            cursor.execute("""
                SELECT player_name, MAX(runs) as high_score
                FROM match_data
                GROUP BY player_name
            ...
            """)
            rows = cursor.fetchall()
            print("\nHIGHEST INDIVIDUAL SCORES".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'HIGH SCORE':<10}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[1]:<10}")
            ...
            cursor.execute("""
                ...
                FROM match_data
                GROUP BY player_name
                ORDER BY win_rate DESC, MIN(timestamp)
                LIMIT 5
            """)
            rows = cursor.fetchall()
            print("\nWIN RATE LEADERBOARD".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'MATCHES':<10}{'WINS':<8}{'LOSSES':<8}{'WIN %':<8}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[1]:<10}{row[2]:<8}{row[3]:<8}{row[4]:<8}")

            cursor.execute("""
                SELECT player_name,
                    SUM(runs) as total_runs,
                    SUM(wickets) as total_wickets,
                    (SUM(runs) + 10 * SUM(wickets)) as allround_score,
                    MIN(timestamp)
                FROM match_data
                GROUP BY player_name
                ORDER BY allround_score DESC, MIN(timestamp)
                LIMIT 5
            """)
            rows = cursor.fetchall()
            print("\nTOP ALL-ROUNDERS".center(60, "-"))
            print(f"{'RANK':<6}{'PLAYER':<15}{'SCORE':<10}")
            print("-" * 60)
            for i, row in enumerate(rows, 1):
                print(f"{i:<6}{row[0].upper():<15}{row[3]:<10}")
        ...
        if player_decision == "odd":
            if role_picked % 2 == 0:
                ...
                while True:
                    ...
                    if player_role == "bat":
                        if self.commentator and self.commentator.enabled:
                            commentator.toss_commentary(self.name, player_role)
                        print(f"\n{self.name} is batting \nAI is bowling")
                        game_var['toss_result'] = "batting"
                        break
        ...
        try:
            ...
            if db_handler.player_exists(name):
                ...
                if reset == "yes":
                    ...
                    db_handler.save_profile(profile, name)
                    game_var.update({
                        "lifetime_runs": 0,
                        "lifetime_wickets": 0,
                        "total_matches_played": 0,
...
def main(game_manager):
    ...
    while True:
        ...
        if choice == "tutorial":
            ...
            time.sleep(2)
            print("\nLeaderboard info:")
            print("TOTAL RUNS SCORED - The lifetime runs of the players ranked.")
            print("TOTAL WICKETS TAKEN - The lifetime wickets of the players ranked.")
            print("BALLS PLAYED - The lifetime balls played of the players ranked.")
            print("MOST ACTIVE PLAYERS - The amount of matches p[alyed/activeness of the players ranked.]")
            print("HIGHEST INDIVIDUAL SCORES - The highest amount of runs obtained in a single match by the players ranked.")
            print("WIN RATE - (WINS/MATCHES) x 100 to rank the winning consistency of the players.")
            print("TOP ALL ROUNDERS - (RUNS + 10) x WICKETS to rank the all rounding playabilty of the players.")
            time.sleep(4)
            db_handler.show_leaderboard()

    ...
    def match_over(self, game_var):
        while True:
            try:
                ...
                sounds.play_click()
                if game_var['over'] < 1:
                    console.print("[grey37]Enter a number greater than 0[/grey37]")
                    continue
    ...
    def second_in(self, game_var):
        ...
        if game_var['toss_result'] == "bowling":
            ...
            for i in range(1, game_var['balls'] + 1):
                ...
                if i % 6 == 0:  
                    current_run_rate = self.calculate_run_rate(game_var['ai_runs_2ndinn'], game_var['balls_played_sec_inn'])
                    required_run_rate = self.calculate_required_run_rate(
...
class GameManager:
    ...
    def show_settings_menu(self, game_var):
        while True:
            ...
            if choice == "1":
                ...
                console.print("[red]Invalid choice.[/red] [grey37]Please select[/grey37] 1, 2, 3, 4, [grey37]or[/grey37] 5")
    ...
    def change_difficulty(self, game_var):
        while True:
            ...
            if difficulty in ("easy", "medium", "hard"):
                ...
                console.print("[red]Invalid choice[/red] [grey37]Try again.[/grey37]")
    ...
    def change_player_name(self):
        ...
        console.print("[green]Enter new player name [/green][grey37](max 30 characters)[/grey37][green]:[/green]")
        new_name = input("").strip().title()
        if not new_name.replace(" ", "").isalpha():
            console.print("[red]Invalid name[/red] [grey37]Only letters and spaces allowed.[/grey37]")
            return
    ...
    def change_password(self):
        ...
        while True:
            ...
            if new_pass != confirm:
                console.print("[red]Passwords do not match.[/red] [grey37]Try again.[/grey37]")
            else:
...
class SoundManager:
    def __init__(self):
        ...
        if self.enabled and os.path.exists("assets/bgmusic.wav"):
            try:
                pygame.mixer.music.load("assets/bgmusic.wav")
            except Exception:
                self.enabled = False  
...
def main(game_manager):
    ...
    while True:
        ...
        if choice == "tutorial":
            ...
            console.print("[bold yellow italic]Thank you for playing[/bold yellow italic] [blue]{name}[/blue][bold yellow italic]![/bold yellow italic]")
            pygame.mixer.music.stop()
            db_handler.close_db()
            break
            ...
            console.print("[yellow italic]Loading save game state...[/yellow italic]")
            time.sleep(3)
            loaded_game = manager.load_game_from_file()
            if loaded_game:
                game_var.update(loaded_game)
                welcome = Panel.fit(f"[bold yellow]Welcome back {name}!![/bold yellow]\n\n[green]Total matches played:[/green] [blue]{game_var['total_matches_played']}[/blue]\n[green]Lifetime runs:[/green] [blue]{game_var['lifetime_runs']}[/blue]", border_style="bright_blue")
                ...
                while True:
                    try:
                        ...
                        if autosave_load_choice == "yes":
                            ...
                            if profile_data:
                                console.print("\b[bold yellow]Profile stats loaded from database.[/bold yellow]")
    ...
    if manager.file_exists():
        ...
        while True:
            try:
                ...
                sounds.play_click()
                if autosave_load_choice == "yes":
                    console.print("[grey37]Loading save game state...[/grey37]")
                    time.sleep(3)
                    loaded_game = manager.load_game_from_file()
                    if loaded_game:
                        game_var.update(loaded_game)
